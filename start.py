...
def proxydl(out_file, socks_type):
    ...
    print("Загрузка {}'s прокси, подождите".format(socktyper))
...
def usge():
    usgeaseets()
    print('* Создано в поле на коленке')
    print('python3 {} <метод> <ссылка> <socks_type5.4.1> <threads> <прокси лист> <multiple> <время>\n'.format(sys.argv[0]))
    print(' > Метод:')
    print(' - флуд')
    print(' | {} | {} Метод'.format(l3s, len(l3)))
    print(' - флуд')
    print(' | {} | {} Метод'.format(l4s, len(l4)))
    print(' - Обходы и ддос')
    print(' | {} | {} Метод'.format(l7s, len(l7)))
    print(' - Инструменты')
    print(' | {} | {} Метод'.format(tos, len(to)))
    print(' - Прочее')
    print(' | {} | {} Метод'.format(ots, len(ot)))
    print(' - All {} Метод \n'.format(len(methodsl)))
    print(
